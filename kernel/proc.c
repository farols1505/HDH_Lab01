...
// load average, stored as fixed-point scaled by 1000
int kernel_loadavg[3] = {0, 0, 0}; // 1,5,15 min
...
update_loadavg(void)
{
  ...
  // fixed-point smoothing; scale = 1000
  // weights approximating Unix's decay (we use simple constants)
  // new = old * alpha + runnable_count * (1 - alpha)
  // Use integer fixed point with scale 1000
  // alpha values for 1,5,15 min approximated for 1-second interval:
